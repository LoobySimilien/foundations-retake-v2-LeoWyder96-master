...
describe('mapRightRecursive', () => {
  beforeEach(() => {
    spyOn(window, 'mapRightRecursive').and.callThrough();
  });
  ...
  it('works with doubler function', () => {
    ...
    const mappedArr = mapRightRecursive([1, 2, 3, 4, 5], doubler);
    expect(mappedArr).toEqual([10, 8, 6, 4, 2]);
    expect(mapRightRecursive.calls.count()).toBeGreaterThan(4);
  });
  ...
  it('works with objects', function() {
    ...
    const mappedArr = mapRightRecursive(books, getPages);
    expect(mappedArr).toEqual([30, 400, 1000]);
    expect(mapRightRecursive.calls.count()).toBeGreaterThan(2);
  });
